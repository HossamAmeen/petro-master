...
class TestUserRetrieve:
    ...
    def test_retrieve_non_admin_fail(self, auth_client, finance_user, customer_support_user):
        response = auth_client(finance_user).get(users_detail_url(customer_support_user.id))

        assert response.status_code == status.HTTP_403_FORBIDDEN


    def test_retrieve_dashboard_user_success(self, auth_client, admin_user, finance_user):
    ...
    def test_retrieve_non_dashboard_user_fail(self, auth_client, admin_user, company_owner):
