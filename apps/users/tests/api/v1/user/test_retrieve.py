...
def test_retrieve_without_authentication_fail(api_client, finance_user):
    response = api_client.get(users_detail_url(finance_user.id))

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


def test_retrieve_non_admin_fail(auth_client, finance_user, customer_support_user):
    response = auth_client(finance_user).get(users_detail_url(customer_support_user.id))

    assert response.status_code == status.HTTP_403_FORBIDDEN


def test_retrieve_dashboard_user_success(auth_client, admin_user, finance_user):
...
def test_retrieve_non_dashboard_user_fail(auth_client, admin_user, company_owner):
    response = auth_client(admin_user).get(users_detail_url(company_owner.id))

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_retrieve_unknown_user_fail(auth_client, admin_user):
