...
def test_retrieve_without_authentication_fail(api_client, branch_manager):
...
def test_retrieve_success(auth_client, admin_user, branch_manager, station, branch):
...
def test_update_name_success(auth_client, admin_user, branch_manager):
...
def test_update_password_mismatch_fail(auth_client, admin_user, branch_manager):
    response = auth_client(admin_user).patch(
        station_branch_managers_detail_url(branch_manager.id),
...
def test_delete_success(auth_client, admin_user, other_station, station):
    from apps.users.models import User
