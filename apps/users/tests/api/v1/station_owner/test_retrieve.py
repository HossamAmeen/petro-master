...
def test_retrieve_without_authentication_fail(api_client, station_owner):
    response = api_client.get(station_owners_detail_url(station_owner.id))

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


def test_retrieve_as_company_owner_fail(auth_client, company_owner, company, station_owner):
...
def test_retrieve_success(auth_client, admin_user, station_owner, station):
    response = auth_client(admin_user).get(station_owners_detail_url(station_owner.id))
