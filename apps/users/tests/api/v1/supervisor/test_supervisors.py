...
def test_list_non_admin_fail(role_fixture, request, auth_client, company, station):
...
def test_list_success(auth_client, admin_user, supervisor, geo_data):
...
def test_create_without_authentication_fail(api_client, supervisor_payload_factory):
...
def test_retrieve_success(auth_client, admin_user, supervisor, geo_data):
...
def test_update_name_success(auth_client, admin_user, supervisor):
...
def test_update_password_mismatch_fail(auth_client, admin_user, supervisor):
    response = auth_client(admin_user).patch(
        supervisors_detail_url(supervisor.id),
...
def test_delete_as_finance_fail(auth_client, finance_user, supervisor):
...
def test_delete_success(auth_client, admin_user, supervisor):
