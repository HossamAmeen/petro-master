...
def test_password_reset_request_sends_email_for_active_user_success(api_client):
    user = UserFactory()
...
def test_password_reset_request_unknown_email_fail(api_client):
    response = api_client.post(
        password_reset_request_url(),
...
def test_password_reset_request_inactive_user_fail(api_client):
...
def test_password_reset_request_invalid_email_fail(api_client):
    response = api_client.post(
        password_reset_request_url(),
        {"email": "not-an-email"},
        format="json",
...
def test_password_reset_request_missing_email_fail(api_client):
...
def test_password_reset_request_send_mail_failure_fail(api_client):
    user = UserFactory()
...
def test_password_reset_request_replaces_existing_token_success(api_client):
    user = UserFactory()
...
def test_password_reset_request_get_method_fail(api_client):
    response = api_client.get(password_reset_request_url())

    assert response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED


def test_password_reset_request_company_owner_success(api_client, company_owner):
