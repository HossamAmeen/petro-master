...
def test_token_refresh_dashboard_success(api_client, admin_user):
...
def test_token_refresh_company_missing_company_id_fail(api_client, company_owner):
...
def test_token_refresh_station_missing_station_id_fail(api_client, station_owner):
...
def test_token_refresh_invalid_token_fail(api_client):
    response = api_client.post(
        token_refresh_url(),
        {"refresh": "not-a-jwt"},
        format="json",
...
def test_token_refresh_missing_refresh_fail(api_client):
    response = api_client.post(token_refresh_url(), {}, format="json")

    assert response.status_code == status.HTTP_400_BAD_REQUEST


def test_token_refresh_access_token_rejected_fail(api_client, company_owner, company):
    refresh = company_refresh_for(company_owner, company.id)
...
def test_token_refresh_rotated_refresh_success(api_client, company_owner, company):
...
def test_token_refresh_get_method_fail(api_client):
