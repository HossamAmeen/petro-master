...
def test_profile_without_authentication_fail(api_client):
    response = api_client.get(profile_url())

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


def test_profile_update_without_authentication_fail(api_client):
...
def test_profile_updates_name_and_email_success(auth_client, company_owner, company):
...
def test_profile_updates_password_success(auth_client, api_client, company_owner, company):
    response = auth_client(company_owner, company_id=company.id).patch(
        profile_url(),
...
def test_profile_empty_patch_success(auth_client, company_owner, company):
    response = auth_client(company_owner, company_id=company.id).patch(
        profile_url(),
        {},
        format="json",
...
def test_profile_post_method_fail(auth_client, company_owner, company):
