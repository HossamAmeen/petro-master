...
def sync_car_balance(car):
    pull_amount = (
        CompanyKhaznaTransaction.objects.filter(
            company=car.branch.company,
            description__icontains=f"{car.plate_character} " + car.car_number,
        )
        .filter(description__icontains="سحب")
        .order_by("created")
    ...
    charge_amount = (
        ...
        .filter(description__icontains="شحن")
        .order_by("created")
        .aggregate(Sum("amount"))["amount__sum"]
    )
    fuel_amount = (
        ...
        .filter(description__icontains="تفويل")
        .order_by("created")
        .aggregate(Sum("amount"))["amount__sum"]
    )
