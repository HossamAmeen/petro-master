...
def test_list_districts_payload_nests_city_success(api_client, geo_data):
...
def test_list_districts_newest_first_success(api_client, geo_data, other_district):
    response = api_client.get(districts_list_url(no_paginate="true"))
...
def test_retrieve_district_success(api_client, geo_data):
    district = geo_data["district"]
    city = geo_data["city"]
...
def test_retrieve_unknown_district_fail(api_client):
    response = api_client.get(districts_detail_url(999_999))

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_create_district_method_not_allowed_fail(api_client, geo_data):
...
def test_update_district_method_not_allowed_fail(api_client, geo_data):
    district = geo_data["district"]
...
def test_delete_district_method_not_allowed_fail(api_client, geo_data):
    district = geo_data["district"]
