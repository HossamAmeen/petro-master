...
def test_list_cities_without_authentication_success(api_client, geo_data, other_city):
...
def test_list_cities_as_authenticated_user_success(auth_client, admin_user, geo_data):
...
def test_list_cities_payload_fields_success(api_client, geo_data):
    city = geo_data["city"]
...
def test_list_cities_filter_by_country_success(
    api_client, geo_data, other_city, other_country
):
...
def test_list_cities_search_by_name_success(api_client, geo_data, other_city):
...
def test_list_cities_newest_first_success(api_client, geo_data, other_city):
    response = api_client.get(cities_list_url(no_paginate="true"))
...
def test_retrieve_city_success(api_client, geo_data):
    city = geo_data["city"]
...
def test_retrieve_unknown_city_fail(api_client):
    response = api_client.get(cities_detail_url(999_999))

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_create_city_method_not_allowed_fail(api_client, geo_data):
...
def test_update_city_method_not_allowed_fail(api_client, geo_data):
    city = geo_data["city"]
...
def test_delete_city_method_not_allowed_fail(api_client, geo_data):
    city = geo_data["city"]
