...
def test_partial_update_without_authentication_fail(api_client, company_branch):
...
def test_partial_update_branch_success(auth_client, admin_user, company_branch):
...
def test_partial_update_fees_success(auth_client, admin_user, company_branch):
    response = auth_client(admin_user).patch(
        branch_detail_url(company_branch.id),
...
def test_partial_update_balance_success(auth_client, admin_user, company_branch):
    response = auth_client(admin_user).patch(
        branch_detail_url(company_branch.id),
        {"balance": "125.50"},
        format="json",
...
def test_partial_update_unknown_branch_fail(auth_client, admin_user):
    response = auth_client(admin_user).patch(
