...
def test_partial_update_without_authentication_fail(api_client, company):
...
def test_partial_update_deactivate_success(auth_client, admin_user, company):
    response = auth_client(admin_user).patch(
        company_detail_url(company.id),
        {"is_active": False},
        format="json",
...
def test_partial_update_unknown_company_fail(auth_client, admin_user):
    response = auth_client(admin_user).patch(
