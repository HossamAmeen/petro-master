...
def test_home_owner_without_company_claim_fail(auth_client, company_owner):
...
def test_home_owner_unknown_company_claim_fail(auth_client, company_owner):
...
def test_home_post_not_allowed_fail(auth_client, company_owner, company):
    response = auth_client(company_owner, company_id=company.id).post(home_url())

    assert response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED


def test_home_owner_empty_company_success(auth_client, company_owner, company):
