...
def test_retrieve_without_authentication_fail(api_client, company_car):
    response = api_client.get(car_detail_url(company_car.id))

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


def test_retrieve_owned_car_success(
    auth_client,
    company_owner,
    company,
    company_car,
...
def test_retrieve_unknown_car_fail(auth_client, admin_user):
