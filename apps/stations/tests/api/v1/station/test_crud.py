...
def test_retrieve_without_authentication_fail(api_client, station):
    response = api_client.get(stations_detail_url(station.id))

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


def test_retrieve_as_dashboard_success(auth_client, admin_user, station, branch):
...
def test_retrieve_as_station_owner_success(auth_client, station_owner, station):
...
def test_create_without_authentication_fail(api_client, station_payload_factory):
...
def test_update_as_admin_success(auth_client, admin_user, station):
...
def test_update_as_station_owner_success(auth_client, station_owner, station):
