...
class TestStationGasOperationAPIView:
    ...
    def test_patch_gas_operation_start_meter_success(self, setup_data, auth_client):
        ...
        data = {"car_meter": 1100, "motor_image": self.mock_image}
    ...
    def test_patch_gas_operation_invalid_meter_fails(self, setup_data, auth_client):
        ...
        data = {"car_meter": 900, "motor_image": self.mock_image}
        response = client.patch(url, data, format="multipart")
    ...
    @patch(
        "apps.stations.api.v1.views.car_operations_views.generate_station_transaction"
    )
    @patch(
        "apps.stations.api.v1.views.car_operations_views.generate_company_transaction"
    )
    def test_patch_gas_operation_complete_success(
        self, mock_comp_tx, mock_station_tx, setup_data, auth_client
    ):
        worker = setup_data
        client = auth_client(worker, station_id=self.station.id)
        url = reverse("station-gas-operations", kwargs={"pk": self.operation.pk})
        ...
        data = {"amount": 10, "fuel_image": self.mock_image}
    ...
    def test_patch_gas_operation_insufficient_balance_fails(
        self, setup_data, auth_client
    ):
        worker = setup_data
        client = auth_client(worker, station_id=self.station.id)
        url = reverse("station-gas-operations", kwargs={"pk": self.operation.pk})
        ...
        data = {"amount": 70, "fuel_image": self.mock_image}
