...
- Tests: the per-action modules for car operations and the three transaction viewsets cover customer support directly; `test_customer_support_read_only.py` in `apps/companies/tests/api/v1/` holds the read-200 / write-403 matrix for the other viewsets and asserts the database is unchanged. `role_client(fixture_name)` in the root `conftest.py` builds a client with the claims that role logs in with, so role matrices can be parametrized over fixture names.
