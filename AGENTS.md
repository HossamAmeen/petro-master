...
- `CompanyViewSet` is authenticated but not role-scoped: any logged-in user can list and retrieve companies, and any logged-in user except customer support can create, update, or delete them. Cover annotated branch/car/driver/manager counts, district/city filters, name/phone search, `no_paginate`, ignored writable-balance, PROTECT deletes when branches or owners exist, and the 400 `has_operations` rejection when any `CarOperation` (any status) belongs to the company's cars.
...
- Users API tests live in `apps/users/tests/api/v1/`, with one package per ViewSet (`user`, `customer_support`, `company_owner`, `company_branch_manager`, `station_owner`, `station_branch_manager`, `worker`, `supervisor`, `firebase_token`). Wrap tests in a `Test*` class; method names end in `_success` or `_fail`.
...
- Tests: the per-action modules for car operations and the three transaction viewsets cover customer support directly; `test_customer_support_read_only.py` in `apps/companies/tests/api/v1/`, `apps/stations/tests/api/v1/`, and `apps/users/tests/api/v1/` hold the read-200 / write-403 matrix for the other viewsets and asserts the database is unchanged. `role_client(fixture_name)` in the root `conftest.py` builds a client with the claims that role logs in with, so role matrices can be parametrized over fixture names.
...
- `StationGasOperationAPIView` PATCH is authenticated only (not worker-scoped; customer support gets 403). Completing `amount` requires `start_time` within 60 seconds, `fuel_image`, and amount ≤ min(tank/permitted, floor(car.balance / company_liter_cost)). It deducts `company_cost` from `car.balance` and `station_cost` from `station_branch.balance` (station/company entity balances are unchanged).
