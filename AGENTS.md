...
- Tests: the per-action modules for car operations and the three transaction viewsets cover customer support directly. `role_client(fixture_name)` in the root `conftest.py` builds a client with the claims that role logs in with, so role matrices can be parametrized over fixture names.
